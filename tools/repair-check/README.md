...
to be wrong in those ways.
    ...
    cargo run --release -p nils-dicom --example awkward -- \
        --out /data/working/awkward > /data/working/awkward-manifest.json

    # digest it
    nils init --registry REG --backend sqlite --scheme blake2b-8 --key k
    nils digest /data/working/awkward --registry REG --files dcm,no-ext,DCM,IMA

    # and see what a correct reader would have found
    python3 tools/repair-check/check.py REG/registry.db /data/working/awkward-manifest.json
