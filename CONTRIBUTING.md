...
- A wave is the unit of work: one decision record, its slices on a branch each, gathered on an integration branch and landed as one pull request where the slices share code, a few where they do not. CI runs per pull request rather than per slice, and no tag is cut for a wave.
- A wave reaches a real machine through a development channel, not through a release. A build of the integration branch is labelled `1.0.0-alpha.N.dev.M`, where `N` is the release it was built from and `M` counts up, and is served on a loopback address the way a release channel is served, with `VERSION`, `SHA256SUMS` and the assets; an install takes it with `nils update --all --channel URL`, so the update path is exercised on every wave. The engine orders such a build after the release it came from and before the next one (`engine/crates/nils/src/update.rs`, `a_release_is_newer_by_its_numbers_then_by_its_pre_release`), so a machine on a development build steps onto the next release when it lands, and never back.
- Where a wave touches the installer, it is proven before any release on a matrix of install shapes in a virtual machine: system units, user units, docker, podman, purge, reinstall and the update path.
- Releases are tags: `v1.0.0-alpha.N` until the first release, `v1.0.0` after. A release closes one wave or several, so an alpha counts a wave rather than a fix, a release is never the way to try something out, and beta comes when the installer and the desk are steady rather than at a count. A tag builds the binaries and publishes the matching section of [`CHANGELOG.md`](CHANGELOG.md) as the release notes, so every pull request that a user would notice adds a line under `Unreleased`. The installer takes Kvasir and the assistant at pinned tags; a wave that changes them points the installer at a branch (`NILS_SETUP_KVASIR_REF`, `NILS_SETUP_ASSISTANT_REF`), and the pin moves in the release.
