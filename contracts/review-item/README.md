...
| 4 | [`v4/review-item.schema.json`](v4/review-item.schema.json) | Wave 4c slice A6, 2026-09-09: `scope` gains `overlay`, the review item beside a proposed overlay (Wave 4c §6.6); the four classifier diagnostics are `diagnostic` rows by kind, not items, so no kind is added. Amended on 2026-09-19 by record 35 slice S6, which adds the `conflict` reason to the kinds the pattern admits: a kind is not a change to this contract, and the pattern had to be told; and on 2026-09-24 by record 43 slice S2, which adds `pipeline:qc` to it the same way, and by record 43 slice S6, which adds the `model` reason for the same cause |
...
| `<axis>:model` | group | a pipeline run's proposals (record 43 S6), one grouped question per axis, model, value and confidence band: `ref` is `{group, run_id}`, `evidence` `{axis, value, tier, confidence, mean_confidence, threshold, model, run_id, members, group}` where `tier` is the band (`below`, or `p>=<edge>` at the threshold and at 0.9, 0.95 and 0.99 above it) and `confidence` the lowest of the members', `group_key` `run:<run>\|<axis>:model\|<value>\|<band>\|model:<id>`; each member's evidence is `{axis, value, confidence, probabilities, model_id, run_id, note}`. A group at or above the threshold is `staged` with a decision by the model, which a person commits; one below it stays open |
