...
| 4 | [`v4/review-item.schema.json`](v4/review-item.schema.json) | Wave 4c slice A6, 2026-09-09: `scope` gains `overlay`, the review item beside a proposed overlay (Wave 4c §6.6); the four classifier diagnostics are `diagnostic` rows by kind, not items, so no kind is added. Amended on 2026-09-19 by record 35 slice S6, which adds the `conflict` reason to the kinds the pattern admits: a kind is not a change to this contract, and the pattern had to be told; and on 2026-09-24 by record 43 slice S2, which adds `pipeline:qc` to it the same way |
