...
| change | `nils campaign create \| claim \| answer \| release \| metric \| close`<br>`nils labels import-v0 --tsv <file>`<br>`nils labels seal --select selection:<name>@<v>`<br>`nils labels certificate --sample <sample> --model <model> --result <file>`<br>`nils labels unseal <sample\|set> --certificate <id>` |
