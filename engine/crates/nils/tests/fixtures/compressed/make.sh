...
# Writes the compressed pyramid fixtures in this folder again: make.py writes
# each plane of planes.txt as a native file, and DCMTK and GDCM, in a Debian
...
grep -v "^#" planes.txt | while read -r name _ _ _ _ how; do
  [ -n "$name" ] || continue
  in="$work/$name.native"
  out="$name.dcm"
  case "$how" in
