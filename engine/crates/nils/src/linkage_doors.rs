...
mod tests {
    ...
    /// The pseudonymiser's table, as the dataset slice declares it.
    fn held_table(registry: &mut Registry) {
        registry
            .store()
            .batch(
                "CREATE TABLE pseudonym_file (id INTEGER PRIMARY KEY, place_id INTEGER, path TEXT, \
                 size INTEGER, mtime INTEGER, state TEXT, shape TEXT, lookup BLOB, sealed BLOB, \
                 id_type TEXT, out_path TEXT, out_size INTEGER, digest TEXT, batch_id INTEGER, \
                 first_seen TEXT, written_at TEXT, released_at TEXT, code_anyway INTEGER DEFAULT 0)",
            )
            .unwrap();
    }
    ...
    fn an_import_dry_runs_here_and_is_queued_from_a_file_of_its_own() {
        ...
        registry
            .store()
            .execute(
                "INSERT INTO pseudonym_file (place_id, path, state, shape, lookup, sealed, id_type, first_seen, batch_id) VALUES (?, 'a', 'held', 'A9', ?, ?, 'patient-id', '2026-09-15T00:00:00Z', 3)",
    ...
    fn the_held_doors_count_code_and_reveal_by_shape() {
        ...
        let mut held = |registry: &mut Registry,
            ...
            registry
                .store()
                .execute(
                    "INSERT INTO pseudonym_file (place_id, path, state, shape, lookup, sealed, id_type, first_seen, batch_id) VALUES (?, ?, ?, ?, ?, ?, 'patient-id', ?, ?)",
                    &[
                        Param::Int(place),
