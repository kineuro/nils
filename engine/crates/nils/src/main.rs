...
enum Command {
    ...
    /// The steps on a registry nils setup takes as the engine's account,
    /// which setup runs itself under runuser
    #[command(hide = true)]
    SetupRegistry {
        #[command(subcommand)]
        command: setup::RegistryStep,
    },
...
fn main() -> ExitCode {
    ...
    let outcome = match cli.command {
        ...
        Command::SetupRegistry { command } => setup::registry_step(&home, command),
...
fn custody_doc(home: &Home, registry: &mut Registry) -> Result<serde_json::Value, Exit> {
    ...
    let certificates = count_of(store, "certificate", "")?;
    ...
    let stores = vec![
        ...
        serde_json::json!({
            ...
            "counts": { "campaigns": campaigns, "items": campaign_items, "answers": campaign_answers, "label_sets": label_sets, "sealed_stacks": sealed_stacks, "certificates": certificates },
            ...
            "commands": {
                ...
                "change": ["nils campaign create | claim | answer | release | metric | close", "nils labels import-v0 --tsv <file>", "nils labels seal --select selection:<name>@<v>", "nils labels certificate --sample <sample> --model <model> --result <file>", "nils labels unseal <sample|set> --certificate <id>"],
