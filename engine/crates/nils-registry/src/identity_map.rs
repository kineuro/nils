...
mod tests {
    ...
    /// The table the dataset slice declares, in the shape the map reads.
    fn held_table(store: &mut Store) {
        store
            .batch(
    ...
    fn an_import_releases_the_held_files_it_names() {
        ...
        let held = |store: &mut Store, id: i64, lookup: &[u8], state: &str| {
            store
                .execute(
                    "INSERT INTO pseudonym_file (id, place_id, path, state, shape, lookup, id_type) VALUES (?, 1, ?, ?, '99', ?, 'patient-id')",
                    &[
                        Param::Int(id),
